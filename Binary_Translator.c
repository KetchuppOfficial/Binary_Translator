...
static const struct Instruction ISA_Consts[N_INSTRUCTIONS] =
{
    ...
    {jne,              0x08,  5, 11, 16},
    {ret,              0x09,  1,  1,  0},
    {in_aligned,       0x0A,  1, 19, -8},
    {in_unaligned,     0x0A,  1, 21, -8},
    {out,              0x0B,  1, 19,  8},
    {push_num,         0x0C, 12, 11, -8},
    {push_ram_num,     0x0C,  8,  9, -8},
    {push_reg,         0x0C,  4,  1, -8},
    {push_ram_reg,     0x0C,  4,  4, -8},
    {push_ram_reg_num, 0x0C,  8,  8, -8},
    {pop,              0x0D,  4,  1,  8},
    {pop_ram_num,      0x0D,  8,  9,  8},
    {pop_reg,          0x0D,  4,  1,  8},
    {pop_ram_reg,      0x0D,  4,  4,  8},
    {pop_ram_reg_num,  0x0D,  8,  8,  8},
    {add,              0x0E,  1, 24,  8},
    {sub,              0x0F,  1, 24,  8},
    {mul,              0x10,  1, 24,  8},
    {dvd,              0x11,  1, 24,  8},
    ...
    for (int ip = 0; ip < max_ip; )
    {
        switch (proc_buff[ip])
        {
            ...
            case je:
            {
                ...
                ip     += ISA_Consts[instruction].proc_sz;
                x86_ip += ISA_Consts[instruction].x86_sz;
                rsp    += ISA_Consts[instruction].delta_rsp;
            ...
            case in:
                ip     += ISA_Consts[instruction].proc_sz;
                x86_ip += (rsp % 16 == 0) ? ISA_Consts[in_aligned].x86_sz : ISA_Consts[in_unaligned].x86_sz;
                rsp    += ISA_Consts[instruction].delta_rsp;
                break;
            ...
            switch (jumps_arr[jump_i].type)
            {
                ...
                case jmp:
                    *(int *)(x86_buff + x86_from + 1) = x86_ip - (x86_from + 1 + sizeof (int));
                    ...
                    #ifdef JUMP_DEBUG
                    printf ("jump_i = %d, relative offset = %zd\n", jump_i, x86_ip - (x86_from + 1 + sizeof (int)));
                    #endif
                ...
                case jne:
                    *(int *)(x86_buff + x86_from + 2) = x86_ip - (x86_from + 2 + sizeof (int));
                    ...
                    #ifdef JUMP_DEBUG
                    printf ("jump_i = %d, relative offset = %zd\n", jump_i, x86_ip - (x86_from + 2 + sizeof (int)));
                    #endif
